...
with st.sidebar:
    ...
    st.markdown(
        ...
        <div class="feature-card">
          ...
          <div class="muted">Use the exported Apple Mail file named <b>mbox</b>. The <b>table_of_contents</b> file is not needed.</div>
          <hr class="soft">
    ...
    sort_order = st.radio(
        ...
        horizontal=False,
        help="Choose how matched messages are displayed.",
    )
...
with left:
    uploaded = st.file_uploader(
        ...
        type=["mbox", "mbx", "mailbox", "txt"],
        help="Choose the Apple Mail export file called 'mbox'.",
    )
...
if do_search:
    ...
    if not search_term.strip():
        ...
        st.warning("Upload the mbox file before searching.")
    else:
        term = st.session_state.search_term
        st.session_state.search_results = [
            item for item in st.session_state.parsed_emails
            if item.second_gts_id == term
        ]
...
if st.session_state.parsed_emails:
    ...
    if st.session_state.search_results:
        ...
        csv_bytes = build_csv(st.session_state.search_results, st.session_state.search_term)

        dl1, dl2 = st.columns([0.24, 0.76])
        with dl1:
            st.download_button(
                "⬇️ Download CSV",
                data=csv_bytes,
    ...
    st.info("Upload the exported mbox file to begin.")
